...
class SymbolPage extends React.Component {
  ...
  handleDelete = rowInfo => {
    ...
    const answer = confirm('Bạn có muốn xóa phần tử này không?');
    if (answer) {
  ...
  handleAdd = () => {
    ...
    if (dialogData.symbol !== '' && dialogData.description !== '' && dialogData.data.length > 0) {
      if (treeData.find(item => item === dialogData.symbol) === undefined) {
        ...
        this.setState({ isEditting: true, openDialog: true });
        this.props.onChangeSnackBar({ status: true, message: 'Ký hiệu chấm công đã tồn tại', variant: 'error' });
      }
    } else {
      this.setState({ isEditting: true, openDialog: true });
      this.props.onChangeSnackBar({ status: true, message: 'Không được bỏ trống các trường bắt buộc', variant: 'error' });
    }
  };
                ...
                return {
                  buttons: [
                    <Fab
                      ...
                      size="small"
                      onClick={() => {
                        console.log(rowInfo.node, "rowInfo.node nè nè")
                        const dialogData = Object.assign({}, rowInfo.node);
                        console.log(dialogData, "dialogData  nè nè")
